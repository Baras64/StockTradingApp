...
public class TradingWindow extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        FlowPane flowPane = new FlowPane(7, 3);
        ...
        borderPane.setMinSize(1025, 500);
        SplitPane pane = new SplitPane();
        pane.getItems().addAll(getVbox(), borderPane);
    ...
    private VBox getVbox() {
        ...
        vbox.getChildren().add(new Label("Base=USD"));
        Label[] currencies = new Label[GetCurrencyData.arrayLength];
        Object[] values = new Object[GetCurrencyData.arrayLength];
        try {
            GetCurrencyData.loadData(values);
        } catch (Exception e) {
            e.printStackTrace();
        }
        for (int i = 0; i < values.length; i++) {
            ...
            currencies[i] = new Label(str.substring(1, index - 1) + " = " + str.substring(index + 1));
        }

        for (Label c : currencies) {
